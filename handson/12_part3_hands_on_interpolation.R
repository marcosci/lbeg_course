...
rainfall <- read.csv("slides/data/rainfall.csv")
rainfall <- rainfall %>% 
  rowwise() %>% 
...
borders = st_read("slides/data/israel_borders.shp")
...
# exkurs parallelisieren
tic()
plan(multisession)
monthly_interpolation2 <- future_map(months, function(i){
  f = as.formula(paste0(i, " ~ 1"))
  v = autofitVariogram(f, as(rainfall, "Spatial"))
  g = gstat(formula = f, model = v$var_model, data = rainfall)
  z = predict(g, grid)
